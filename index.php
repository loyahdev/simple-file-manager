...
$base_dir = '/srv/share'; // set base directory, something like '/.git'
...
<head>
    ...
    <script>
        ...
        $(function () {
            ...
            // Assuming that '/srv/share' is directly accessible via http://yourdomain.com/share/
            // You would need to set up an alias in your web server configuration for this to work
            // The alias would map '/srv/share' to 'http://yourdomain.com/share/'

            function renderFileRow($data) {
                $isDir = $data['is_dir'] ? 'is_dir' : '';
                $name = htmlspecialchars($data['name']);
                $size = htmlspecialchars(formatFileSize($data['size']));
                $mtime = htmlspecialchars(formatTimestamp($data['mtime']));
                $path = urlencode($data['path']);
                $permissions = implode('+', array_filter(['read' => $data['is_readable'], 'write' => $data['is_writable'], 'exec' => $data['is_executable']]));
                $permissions = htmlspecialchars($permissions);

                $downloadLink = $data['is_dir'] ? '#' : "/share/$path"; // This needs to be a URL path, not a filesystem path.
                $link = "<a href=\"$downloadLink\" class=\"name\">$name</a>";

                $html = "<tr class=\"$isDir\">";
                $html.= "<td class=\"first\">$link</td>";
                $html.= "<td>{$size}</td>";
                $html.= "<td>{$mtime}</td>";
                $html.= "<td>$permissions</td>";
                // Add other cells for actions like download, edit, delete as needed.
                $html.= "</tr>";
...
<body>
    ...
    <div id="whole" style="position:absolute; z-index  : 10; top:50%; left:50%; transform: translate(-50%, -50%);">
        <div style="display:inline-block;">
            <div style="">
                <textarea style="display:block; margin-bottom:5px" id="textarea"
                    wrap="hard">Type some text here.</textarea>
                <button class="control" style="display:inline-block; float:right"
                    onclick="close_editor()">Cancel</button>
                <button class="control" style="display:inline-block; float:right" onclick="ajaxSave()">Save</button>
            </div>
        </div>
    </div>
    <footer>simple php filemanager by <a href="https://github.com/jcampbell1">jcampbell1 modified by <a
                href="https://github.com/diego95root">diego95root</a></footer>
</body>
